...
def check_buttons():
    """
    Check which button the participant wants to select by analyzing pixel changes
    in the designated button areas. This function handles transitions between modes.
    """
    ...
    if stat_flag == 0:  # Menu mode
        ...
        curr_mean_game = np.array(cv2.mean(game_button_area)[:-1])  # Mean color over the game button area
        curr_mean_learn = np.array(cv2.mean(learn_button_area)[:-1])  # Mean color over the learn button area

        # Check if the game button is pressed
        ...
        # Check if the learn button is pressed
    ...
    if stat_flag == 1 or stat_flag == 2:  # Learn or game mode
        ...
        curr_mean_exit = np.array(cv2.mean(exit_button_area)[:-1])  # Mean color over the exit button area
        # Check if the exit button is pressed
        if np.linalg.norm(curr_mean_exit - init_mean_exit) > 40:
            run_menu_flag = False
            stat_flag = 0  # Move to menu mode
            back2menu.play()
            clear_window(window)
...
def init_button(frame):
    """
    Calculate the initial pixel values in the button areas for comparison.
    This helps in detecting button presses based on pixel value changes.
    """
...
def read_word_and_pre_pros(image_path, factor):
    """
    Load an image from the specified path, resize it according to the factor,
    and convert it to a format suitable for display in Tkinter.
    """
...
def extract_hog_features(image):
    """
    Extract HOG (Histogram of Oriented Gradients) features from the input image.
    This is a common feature extraction technique used in image processing.
    """
...
def predict_func(frame_copy):
    """
    Predict the movement performed by the participant using a pre-trained model.
    This function processes the frame to identify significant features for prediction.
    """
    ...
    # Second step: Finding significant corners using Harris corner detection Algorithm.
    ...
    # Check if the prediction is significant
...
def main_menu(window):
    """
    Main menu function where different modes (learn or game) are handled.
    Displays the current frame from the camera and processes user inputs.
    """
    ...
    init_button(frame)  # Initialize button states
    displaying_the_modes()  # Display buttons on screen
    run_menu_flag = True
    move_timer = time.time()

    # Initial setup for learning or game mode
    if stat_flag == 1:  # Learning mode
        ...
        pred_tik = time.time()
        predicted_labels = []
        ind_to_predict = 0  # Index for the current word to learn
...
def clear_window(window):
    """
    Clears all widgets from the main window to reset the interface.
    """
...
if __name__ == "__main__":  # real time only, this open the camera!
    ...
    stat_flag = 0  # 0: main menu, 1: learn mode, 2: game mode
    window = tk.Tk()
    ...
    ret, frame = cap.read()  # Start camera feed
    # Load the pre-trained Logistic Regression model
    model_path = r'LogisticRegression_12_(16, 16)_(4, 4).pkl'
    model = joblib.load(model_path)
    main_menu(window)  # Start the application
